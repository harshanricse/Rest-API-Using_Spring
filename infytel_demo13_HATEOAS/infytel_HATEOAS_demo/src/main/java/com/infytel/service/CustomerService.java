	...
	//Fetches the existing set of customers
		...
		public CustomerDTO fetchCustomerById(int customerId)throws CustomerNotFoundException
		{
			...
			if(customerOptional.isPresent())
			{
				...
				ResponseEntity<PlanDTO> plan = restTemplate.getForEntity(
									"http://localhost:9000/infytel/plans/"+planId, PlanDTO.class);
			...
			{
				throw new CustomerNotFoundException(environment.
