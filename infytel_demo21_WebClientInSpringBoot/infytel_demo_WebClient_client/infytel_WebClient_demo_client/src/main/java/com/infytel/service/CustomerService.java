...
//	private RestTemplate restTemplate;
	...
	public CustomerDTO createCustomer(CustomerDTO customerDTO)
	{
		...
		PlanDTO planDTO =builder.build().get().uri("http://localhost:8080/infytel/plans/"+customer.getPlanId()).retrieve().bodyToMono(PlanDTO.class).block();    //Using WebClient
		System.out.println("Step1");
		//customerDTOReceived.setPlanDTO(planEntity.getBody());
		System.out.println(customerDTOReceived);
		customerDTOReceived.setPlanDTO(planDTO);
		customerRepository.save(modelMapper.map(customerDTOReceived, Customer.class));
...
//using REST Template				//ResponseEntity<PlanDTO> plan = restTemplate.getForEntity("http://localhost:8080/infytel/plans/"+planId, PlanDTO.class);
				PlanDTO planDTO =builder.build().get().uri("http://localhost:9002/infytel/plans/"+planId).retrieve().bodyToMono(PlanDTO.class).block();    //Using WebClient
